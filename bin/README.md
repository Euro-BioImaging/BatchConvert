...
In this example, the increments in the variable field are non-uniform. BatchConvert will
split this dataset into two groups and create two OME-Zarrs with the following names:
...
```
Try converting this folder to a single OME-Zarr, concatenating the files along the dimensions
detected from the filenames: \
`batchconvert --omezarr --merge_files <input_path>/multichannel_time-series <output_path>`

In this example the channel-2 does not have the same number of timeframes as the channel-1. 
...
```
If you repeat the same conversion on this version of the input folder, the output will be
a single OME-Zarr folder with the name: \
...
the first group lack any dimension specifier. BatchConvert allows the user to 
specify the concatenation axes via `--concatenation_order` option. This
...
(T and Z as specified in the filenames preceding the variable fields),
