    ...
    if ( params.source_type == "local" ) {
        ...
        if ( params.merge_files == "True" ) {
            ...
            if ( params.in_path.contains( "*" ) ) {
                ...
                output = Convert_Concatenate2SingleOMETIFF(ch, imgpath)
            }
        }
        else {
            if ( is_csv( fpath.toString() ) ) { // local && csv && ! merge_files
